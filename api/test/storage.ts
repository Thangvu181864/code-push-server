...
import { SqlS3Storage } from "../script/storage/sql-s3-storage";
...
describe("S3 and SQL Storage", () => storageTests(SqlS3Storage));
...
function storageTests(StorageType: new (...args: any[]) => storageTypes.Storage, disablePersistence?: boolean) {
  ...
  describe("Storage management", () => {
    it("should be healthy if and only if running Azure storage", () => {
      ...
      //   },
      //   /*returnedUnhealthy*/ () => {
      //     assert.equal(StorageType, SqlS3Storage, "Should only return unhealthy if running SQL/S3 storage");
      //   }
      // );
    });
    ...
    if (StorageType === AzureStorage) {
      it("should allow reconfiguring of Azure storage credentials", () => {
        const azureStorage: AzureStorage = storage as AzureStorage;
        return azureStorage
  ...
  describe("Access Key", () => {
    ...
    it("can retrieve an access key by id", () => {
      ...
      return storage
        ...
        .then((accessKeyId: string): q.Promise<storageTypes.AccessKey> => {
    ...
    it("can retrieve the account id by the access key name", () => {
      ...
      return storage
        .addAccessKey(account.id, accessKey)
        .then((accessKeyId: string): q.Promise<string> => {
    ...
    it("can remove an access key", () => {
      ...
      return storage
        .addAccessKey(account.id, accessKey)
        .then((accessKeyId: string): q.Promise<void> => {
          return storage.removeAccessKey(account.id, accessKeyId);
        })
        .then((): q.Promise<storageTypes.AccessKey> => {
    ...
    it("can update an access key", () => {
      ...
      return storage
        .addAccessKey(account.id, accessKey)
        .then((addedAccessKeyId: string): q.Promise<void> => {
  ...
  describe("DeploymentInfo", () => {
    ...
    beforeEach(() => {
      ...
      return storage
        .addAccount(account)
        .then((accountId: string): q.Promise<storageTypes.App> => {
          account.id = accountId;

          return storage.addApp(account.id, app);
        })
        .then((addedApp: storageTypes.App): q.Promise<string> => {
          app.id = addedApp.id;
  ...
  describe("Package", () => {
    ...
    describe("Package history", () => {
      ...
      beforeEach(() => {
        ...
        let promiseChain: q.Promise<void> = q<void>(null);
        let packageNumber = 1;
        for (let i = 1; i <= 3; i++) {
